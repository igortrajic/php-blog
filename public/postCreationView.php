<?php
function renderPostCreation(array $postData, array $categories, string $message = ''): void
{
    include 'header.php';
?>
        ...
        <div class="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm">
            ...
            <form action="postCreation.php" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="title" value="<?= htmlspecialchars($postData['title']) ?>" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-4 focus:ring-blue-50/50 focus:border-blue-500 outline-none" placeholder="Enter a catchy title...">
                </div>
                <div>
                    ...
                    <select name="category_id" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-blue-500 outline-none">
                        ...
                        <?php foreach ($categories as $category): ?>
                            <option value="<?= (int)$category['id'] ?>" <?= (int)$category['id'] == $postData['category_id'] ? 'selected' : '' ?>>
                ...
                <div>
                    ...
                    <textarea name="content" rows="8" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-4 focus:ring-blue-50/50 focus:border-blue-500 outline-none" placeholder="Write your story..."><?= htmlspecialchars($postData['content']) ?></textarea>
                </div>
